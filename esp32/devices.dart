...
class _DeviceListState extends State<DeviceListScreen> {
  ...
  void _registerDevice(BuildContext context) async {
    ...
    DocumentReference pendingRef = FirebaseFirestore.instance.collection('pending').doc(deviceId);
    pendingRef.set(device);

    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Registering $deviceId')
    ));
  ...
  Widget _queryDeviceList(User user) {
    if (user == null) {
          ...
          default:
            return ListView(
              children: snapshot.data.docs.map((DocumentSnapshot data) {
                Device device = Device.fromData(data.data(), data.id);
                return ListTile(
