...
string print_help() {  
  ...
  helpText = helpText + F("datalog      Takes readings of all sensors every 15 sec send to thingspeak ") + '\n';
  helpText = helpText + F("             Entering any commands stops datalog mode.") + '\n';
  helpText = helpText + F("poll         Takes readings of all sensors every second") + '\n';
  
  helpText = helpText + F("rtd:cal,clear        clear calibration") + '\n';
  helpText = helpText + F("rtd:cal,t            calibrate the temp probe to any temp value") + '\n';

  helpText = helpText + F("ec:cal,clear         clear calibration") + '\n';
  if (k_val > 9) {
    helpText = helpText + F("ec:k,10         You have a K10 probe") + '\n';
  }
  else if (k_val > .9) {
    helpText = helpText + F("ec:k,1         You have a K1 probe") + '\n';
  }
  else if (k_val > .09) {
    helpText = helpText + F("ec:k,0.1         You have a K0.1 probe") + '\n';
  }
  
  helpText = helpText + F("ec:cal,dry           calibrate a dry EC probe") + '\n';
  helpText = helpText + F("ec:cal,2760     calibrate EC probe to 2,76 cms  i.e the one from shop") + '\n';

  helpText = helpText + F("ph:cal,clear     clear calibration") + '\n';
  helpText = helpText + F("ph:cal,mid,7     calibrate to pH 7") + '\n';
  helpText = helpText + F("ph:cal,low,4     calibrate to pH 4") + '\n';
  helpText = helpText + F("ph:cal,high,10   calibrate to pH 10") + '\n';
  
  helpText = helpText + F("do:cal,clear         clear calibration") + '\n';
  helpText = helpText + F("do:cal               calibrate DO probe to the air") + '\n';
  helpText = helpText + F("do:cal,0             calibrate DO probe to O dissolved oxygen") + '\n';
  return helpText;
}
