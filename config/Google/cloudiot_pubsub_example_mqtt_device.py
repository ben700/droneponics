...
on how to do this. Run this script with the corresponding algorithm flag.
  ...
  $ python cloudiot_pubsub_example_mqtt_device.py \
      ...
      --registry_id=droneDevice \
      --device_id=pixelbook \
      --private_key_file=pixelbook_rsa_private.pem \
      --algorithm=RS256
...
class Device(object):
    ...
    def on_message(self, unused_client, unused_userdata, message):
        ...
        if data['pH_dose'] != self.pH_dose:
            # If changing the state of the fan, print a message and
            # update the internal state.
            self.pH_dose = data['pH_dose']
            if self.pH_dose:
                print('pH dose turned on.')
            else:
                print('pH dose turned off.')

        if data['ec_dose'] != self.ec_dose:
            # If changing the state of the fan, print a message and
            # update the internal state.
            self.ec_dose = data['ec_dose']
            if self.ec_dose:
                print('EC dose turned on.')
            else:
                print('EC dose turned off.')
...
def main():
    ...
    for _ in range(args.num_messages):
        ...
        # as a JSON string.
#        buff = "{"+"'pH': {}  , 'conductivity': {}".format(device.pH, device.conductivity)+ "}"
 #       print(buff)
        payload = json.dumps({'pH': device.pH  , 'conductivity': device.conductivity })
