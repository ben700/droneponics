...
#include "droneponics_config.h" // Wifi configuration here
...
void logDroneponicsCommandCallback(String data){
     Serial.print("data = " + data.substring(0,3) +"\n");
     if(data.substring(0,3) == "cal"){
       ...
       Serial.print("calDevice =" + calDevice + "\n");
       Serial.print("calCommand = " + calCommand + "\n");
Serial.print("PH.i2c_address");
//  Serial.print(PH.get_address());
//       if(calDevice == PH.get_address()){
...
//  return publishState(calError("Device Id"));
...
{
  //Serial.println("publishBootDataTelemetry=" + data);
  mqtt->publishTelemetry(deviceBootTopic, data);
  return false;
}
...
bool publishSensorDataTelemetry(String data)
{
 // Serial.println("publishSensorDataTelemetry=" + data);
  mqtt->publishTelemetry(sensorReadingTopic, data);
  return false;
}
